            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('category.update','test')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col">
                                <label class="mb-1">الفئه الاساسيه ( أن وجد )</label>
                                <select class="form-control" name="parent_id">
                                    <option value="" disabled selected>-- أختر من القائمه --</option>
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" {{$category->id == $row->parent_id ? 'selected':null}}>({{$category->name_ar}}) /
                                            ({{$category->name_en}})
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            <div class="col">
                                ...
                                @if($row->image)
                                    ...
                                    <img src="{{asset('storage/category/'.$row->image)}}" width="100px" height="100px" alt="">
                                @endif
